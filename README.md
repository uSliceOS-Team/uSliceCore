...
A lightweight, platform-independent cooperative scheduler built around a single main loop and finite-state machines. Designed for systems where predictable task execution matters.
...
µSliceCore replaces preemptive scheduling with a deterministic main loop. On each iteration, it traverses all registered tasks in the order established during static initialization and gives each task one scheduler turn. There is no context switching, no separate stack per task, and no scheduler that can interrupt task execution at an arbitrary point.
...
This architecture is useful when task execution must remain serialized and lifecycle timing must be easy to inspect. If the worst-case execution time of every task handler and the relevant scheduler and interrupt overhead are known, the maximum scheduler-induced delay between two turns of the same task can be bounded by one full traversal of the task list.
...
One rule: every task step must be short relative to your system's latency budget. Do not use blocking calls or busy-wait loops. A long-running step delays every task that is waiting for its next turn.
...
- The maximum scheduler-induced delay between two turns of the same task is one full traversal of the task list. Its duration can be measured and bounded when the execution time of every handler and the relevant scheduler and interrupt overhead are bounded.
- There is no context-switching overhead, and no stack is allocated per task.
- Task startup and shutdown sequencing is deterministic (see [Task Lifecycle Timing](#task-lifecycle-timing)).
...
Startup and shutdown timing follow fixed rules. A stop request has the same timing whether it comes from the task itself through `STOP_SELF()` or from other code through `STOP_TASK()`.
...
It helps to stop thinking in terms of "this pass" vs. "next pass" and instead picture the scheduling loop as an infinite sequence of turns, one per task, repeating forever: `... T1 T2 T3 T1 T2 T3 T1 T2 T3 ...`. In that view, there is only one rule: **cleanup runs at the very next occurrence of that task in the sequence.** Whether that occurrence is labeled "same pass" or "next pass" in a diagram depends only on where you draw the pass boundary relative to the two tasks involved. The guarantee is the same either way: cleanup always occurs within one full traversal of the task list, which is the same bound used throughout this scheduler. The `Guard` turn above provides the same guarantee at the other end of the lifecycle.
...
The host-side test suite covers this timing (both workflows, cleanup deferral, and the no-op windows below), and you can run it before touching real hardware -- see [`tests/`](tests).
...
Tasks register themselves in a linked list during static initialization, prepending themselves to the head. The scheduler holds a pointer to the head and traverses the list during each iteration. The scheduler and time modules perform no dynamic memory allocation. User-provided task handlers remain outside this guarantee.
...
- Cooperative main-loop scheduling with no scheduler priorities or task
  preemption. Pass duration can be bounded when the execution time of every
  handler and relevant interrupt is bounded (see [Architecture](#architecture)).
...
- Deterministic task handover and lifecycle timing
