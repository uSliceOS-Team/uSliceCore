...
#define REGISTER_TASK(instance_name, func_name, CtxType, autostart)            \
    ...
    inline Task instance_name{#instance_name,       Entry_##func_name,         \
                              Loop_##func_name,     Stop_##func_name,          \
                              &instance_name##_ctx, autostart};                \
    }                                                                          \
    }
...
/// Register a context-less task (no state to preserve between calls), starting
/// stopped.
...
#define ADD_TASK_NO_CTX_AND_START(name)                                        \
    ...
    namespace OS {                                                             \
    namespace Tasks {                                                          \
    inline Task name{#name,       Entry_##name, Loop_##name,                   \
                     Stop_##name, nullptr,      true};                         \
    }                                                                          \
    }
