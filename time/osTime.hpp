/**
 * @file osTime.hpp
 * @brief Lightweight software timer engine for cooperative multitasking in
 * embedded systems.
...
public:
     ...
     *
     * Uses unsigned arithmetic logic `(now - start) >= period` to strictly
...
public:
     ...
     */
    [[nodiscard]]
    uint32_t getMs(void) const {
        return (Core::getSystemTick() - startTime);
    }
     ...
     */
    [[nodiscard]]
    uint32_t getS(void) const {
        return ((Core::getSystemTick() - startTime) / 1000);
    }
     ...
     */
    [[nodiscard]]
    uint32_t getM(void) const {
        return ((Core::getSystemTick() - startTime) / 60000);
    }
    ...
    uint32_t getH(void) const {
        return ((Core::getSystemTick() - startTime) / 3600000);
    }
};
} // namespace Time
} // namespace OS
